...
par(mfrow = c(1, 1))
  ...
  for (truncation_distance in truncation_distance_list) {
    ...
    print(paste0("Truncation distance: ", truncation_distance,"m"))
    # Loop through bin sizes
    for (i in seq(from = 0.1, to = 0.1, by = 0.01)) {
      # Create detection function models with bins
      print(paste0("Bin size: ", i,"m"))
      ...
      detfc_list_bin[[paste0("detfc_hn_cos_trunc_", truncation_distance, "_bin_", i)]] <- possDS(distdata, truncation_distance, key = "hn", adjustment = "cos", max_adjustments = 2,  monotonicity = 'strict', cutpoints = seq(from = 0, to = truncation_distance, by = i))
    }
  }
  ...
  for (truncation_distance in truncation_distance_list) {
    ...
    for (i in seq(from = 0.1, to = 0.1, by = 0.01)) {
      # Create detection function models with bins
      # detfc_list_bin[[paste0("detfc_cc_hr_null_trunc_", truncation_distance, "_bin_", i)]] <- possDS(distdata, truncation_distance, formula = ~as.factor(canopy_cover), key = "hr", adjustment = NULL, cutpoints = seq(from = 0, to = truncation_distance, by = i))
      detfc_list_bin[[paste0("detfc_cc_hn_null_trunc_", truncation_distance, "_bin_", i)]] <- possDS(distdata, truncation_distance, formula = ~as.factor(canopy_cover), key = "hn", adjustment = NULL, cutpoints = seq(from = 0, to = truncation_distance, by = i))
      detfc_list_bin[[paste0("detfc_cc_hn_cos_trunc_", truncation_distance, "_bin_", i)]] <- possDS(distdata, truncation_distance, formula = ~as.factor(canopy_cover), key = "hn", adjustment = "cos", max_adjustments = 2,  monotonicity = 'strict', cutpoints = seq(from = 0, to = truncation_distance, by = i))
    }
  }
...
for (model_name in model_metrics$Model) {
  model <- detfc_list_bin[[model_name]]
...
par(mfrow = c(1, 1))
  ...
  best_model <- detfc_list_bin[[names(sorted_aic_diff)[4]]]
...
for (i in 1:length(detfc_list_bin_compact)) {
  ...
  save(dsm_list, detfc_list_bin_compact, file = output_path)
