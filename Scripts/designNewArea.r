...
# Load density data
wmu_shp <- st_read("D:\\Sync\\2_Areas\\MooseProject\\Data\\WMU\\base_data\\WMU\\wmu_354_3400.shp")
...
region <- make.region(
  ...
  shape = wmu_shp,
  strata.name = wmu_shp$WMUNIT_N_1
)
...
### Coverage
QC_Sys_design <- run.coverage(QC_Sys_design, reps = COV_REPS)

QC_Sys_nadir_design <- QC_Sys_design
QC_Sys_nadir_design@truncation <- 50
QC_Sys_nadir_transects <- generate.transects(QC_Sys_nadir_design)
QC_Sys_nadir_design <- run.coverage(QC_Sys_nadir_design, reps = COV_REPS)
...
output_path <- here("Output", "Simulation", paste0("cover-WMU354.RData"))
