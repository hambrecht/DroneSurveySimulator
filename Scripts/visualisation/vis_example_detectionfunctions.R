...
# Plot the data
plot(detect_FW2_data$distance, detect_FW2_data$detectability, type = "l", col = custom_colors[1], lwd = 2, xlim = c(0, 600), ylim = c(0, 1), xlab = "Distance", ylab = "Detectability")
lines(detect_FWG_data$distance, detect_FWG_data$detectability, col = custom_colors[2], lwd = 2, lty = 2) # Dotted line
lines(detect_uf_data$distance, detect_uf_data$detectability, col = custom_colors[3], lwd = 2)
lines(detect_H_data$distance, detect_H_data$detectability, col = custom_colors[4], lwd = 2)

# Add a legend
legend("topright", legend = c("2 Cameras", "Gimbal", "NADIR", "Helicopter"), col = custom_colors, lty = c(1, 1, 2, 1), lwd = 2, cex = 0.9, box.lwd = 1, box.col = "NA", bg = "white")
