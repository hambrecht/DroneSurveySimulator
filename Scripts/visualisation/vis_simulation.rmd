...
histogram.N.ests(FW_Sys_2C_sim, xlim = c(round(sum(sim_fixW_sys_2C@population.description@N)) * 0.9, round(sum(sim_fixW_sys_2C@population.description@N)) * 1.05))
usr <- par("usr")
text(x = usr[2], y = usr[4], labels = labels[6], adj = c(3.0, 0.9), col = "black", cex = 1.2, font = 2)

histogram.N.ests(FW_ZZ_2C_sim, xlim = c(round(sum(sim_fixW_zigzag_2C@population.description@N)) * 0.9, round(sum(sim_fixW_zigzag_2C@population.description@N)) * 1.05))
usr <- par("usr")
text(x = usr[2], y = usr[4], labels = labels[7], adj = c(3.0, 0.9), col = "black", cex = 1.2, font = 2)

histogram.N.ests(FW_Sys_G_sim, xlim = c(round(sum(sim_fixW_sys_G@population.description@N)) * 0.9, round(sum(sim_fixW_sys_G@population.description@N)) * 1.05))
usr <- par("usr")
text(x = usr[2], y = usr[4], labels = labels[8], adj = c(3.0, 0.9), col = "black", cex = 1.2, font = 2)

histogram.N.ests(FW_ZZ_G_sim, xlim = c(round(sum(sim_fixW_zigzag_G@population.description@N)) * 0.9, round(sum(sim_fixW_zigzag_G@population.description@N)) * 1.05))
usr <- par("usr")
text(x = usr[2], y = usr[4], labels = labels[9], adj = c(3.0, 0.9), col = "black", cex = 1.2, font = 2)

histogram.N.ests(QC_Sys_sim, xlim = c(round(sum(sim_quad@population.description@N)) * 0.7, round(sum(sim_quad@population.description@N)) * 1.05))
usr <- par("usr")
...
averaged_df <- comparison_df %>%
  ...
  summarize(
    ...
    relative_Mean_SE = round(mean(relative_Mean_SE, na.rm = TRUE), 3),
...
```
```{r}
# Box plot for Percent Bias
ggplot(comparison_df, aes(x = Simulation, y = Percent_Bias)) +
  geom_boxplot() +
  labs(title = "Box Plot of Percent Bias by Simulation", x = "Simulation", y = "Percent Bias") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Box plot for CV
ggplot(comparison_df, aes(x = Simulation, y = CV)) +
  geom_boxplot() +
  labs(title = "Box Plot of CV by Simulation", x = "Simulation", y = "CV") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
